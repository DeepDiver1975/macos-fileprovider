...
* [~] **Task 7.5:** the Mac-only `DomainService` adapter over `NSFileProviderManager`:
  ...
  * **Remaining:** the live `NSFileProviderManager.add/remove/getDomains` calls only run on
    a signed, installed host (Task 6.0/6.3).
...
* [~] **Task 7.8:** the settings window — the app's *main* window, not a separate
  ...
  * **Remaining:** the sign-in flow itself (Classic username/password, oCIS OIDC via
    `ASWebAuthenticationSession`) is the outstanding Mac-only UI — `beginAddAccount` is the
    seam, and diagnostics/reset are wired but exercised only on a signed host.
...
* [~] **Task 7.11:** the Classic sign-in flow that closes the Task 7.8 gap. Before this the
  ...
  * **Mac-gated UI (built, compiles clean — GUI-only, needs a human at the machine):** the
    `SettingsModel.addAccount` wiring and the `AddAccountSheet` view. The one remaining manual
    check is the Finder round-trip: `make install`, launch, Add Account `http://localhost:8080`
    admin/admin, approve the extension in System Settings, confirm the domain mounts and Sign
    Out removes it. This cannot be automated headlessly (System Settings approval + Finder are
    GUI-gated), so it stays a documented manual step like the rest of Phase 7's live remainders.
