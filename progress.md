...
* `[~]` **Task 9.6 — the workflow.** `.github/workflows/release.yml` on `push:
  ...
  on the six repository secrets, none of which exist yet**, so no release has ever
  run; this stays `[~]` until a throwaway tag has been pushed end to end. Tasks 9.7
