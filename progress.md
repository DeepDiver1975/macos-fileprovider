...
* [~] **Task 1.1:** Create the **Xcode project** with three targets: the containing app, the File Provider extension (`.appex`), and the FileProviderUI extension (`.appex`). SPM has no product type that emits an `.appex`, so the extensions cannot be SPM targets (build-system behaviour, not something Apple's API reference documents — reconfirm on the Mac). Place backend-agnostic core logic (WebDAV XML parsing, Graph models, credential/session management) in a **local SPM package** consumed by all three targets.
  * **Loop status:** local SPM package `Core/` (`OwnCloudCore`) created and green under `swift test`; per-target source trees (`App/`, `FileProviderExtension/`, `FileProviderUIExtension/`) and the intended target/dependency wiring committed and documented in [`PROJECT.md`](PROJECT.md). **Blocked:** the actual `.xcodeproj` with the two `.appex` targets must be assembled in Xcode on a Mac (no `xcodegen`/`tuist` in the loop env; a hand-written `.pbxproj` would be unreviewable).
* [~] **Task 1.2:** Configure `Info.plist` for **each** extension target separately:
  ...
  * **Loop status:** all three `Info.plist` files written with the correct `NSExtensionPointIdentifier` values (`com.apple.fileprovider-nonui`, `com.apple.fileprovider-actionsui`), principal-class keys, and no `NSFileProviderDomainUsageDescription`. **Blocked:** the two identifier strings still need the Task 1.2 reconfirmation against the `.xctemplate` inside `Xcode.app` on the Mac.
...
* [~] **Task 1.4:** Establish the test harness: `swift test` covers the local SPM core package; the extension targets are covered by XCTest targets inside the Xcode project (`xcodebuild test`).
  * **Loop status:** `swift test` over `Core/` runs green (one smoke test today; Phase 2 fills it in). **Sandbox caveat recorded in [`PROJECT.md`](PROJECT.md):** the Xcode module cache lives outside the default command sandbox, so `swift test` must run with the sandbox disabled in this environment. **Blocked:** the `xcodebuild test` half needs the `.xcodeproj` (Task 1.1) on a Mac.
...
* **Acceptance gate:** the enumeration scenario group — items appear in the domain without being downloaded, server-side additions/modifications/deletions are reflected, and a folder large enough to force pagination enumerates completely.
...
* **Acceptance gate:** the content scenario group — hydration on read, upload of locally created files, rename/move, delete, the conflict scenario, and the large-file and many-files throughput scenarios.
...
* [ ] **Task 5.3:** Run end-to-end integration test suite validating Finder synchronization performance and error recovery states. This is the Phase 6 acceptance suite, not a second one.
* **Acceptance gate:** the lifecycle scenario group — domain add and removal (including the `NSFileProviderManager.DomainRemovalMode` choice of what happens to local files), sign-in and re-authentication after credential expiry, and behaviour while the domain is disconnected (`NSFileProviderDomain.isDisconnected`).
...
* **AC-4 — Reproducible locally, identically.** A single entry point —
  `make acceptance BACKEND=classic|ocis` — brings up the Docker fixtures and runs exactly
  the scenarios CI runs. No CI-only wiring.
