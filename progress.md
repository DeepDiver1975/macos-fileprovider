...
* [~] **Task 7.13:** **oCIS OIDC sign-in reachable from the settings UI** (GitHub issue
  ...
  * **Two unrelated pre-existing bugs this surfaced — NOT issue #17, deliberately not fixed
    ...
    write verb is wrong. That is follow-up work of its own, tracked under Task 4.4.
  * **Remaining (keeps `[~]`):** issue #17's own scope — sign-in reachable from the settings
    ...
    **complete and live-proven**. What is still unproven is the **server-side round trip
    through that mount**, and it is unproven because it is *broken*: the two oCIS push bugs
    above mean nothing written into the mount arrives. That fix belongs to Task 4.4, not
    here. Two housekeeping defects also remain noted and unfixed: a stale `spaces.…`
    app-group key survives Sign Out, and old `ownCloudFileProvider-*` replica directories
    accumulate (and, being reused by display name, can silently confound a live check — see
    the discarded first attempt above).
