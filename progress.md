...
* [~] **Task 7.13:** **oCIS OIDC sign-in reachable from the settings UI** (GitHub issue
  ...
  * **Remaining (keeps `[~]`):** issue #17's own scope — sign-in reachable from the settings
    ...
    **complete and live-proven**, and the upload round trip that used to block this marker is
    ...
    by independent `curl`). Two housekeeping defects remain noted and unfixed: a stale
    `spaces.…` app-group key survives Sign Out, and old `ownCloudFileProvider-*` replica
    directories accumulate (and, being reused by display name, can silently confound a live
    check — see the discarded first attempt above). A third, learned the hard way in 4.5:
