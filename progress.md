...
* [~] **Task 7.11:** the Classic sign-in flow that closes the Task 7.8 gap. Before this the
  ...
  * **Mac-gated (built, compiles clean):** `HTTPServerProbe` (GET
    `/.well-known/openid-configuration` → OIDC signal; GET `/status.php` → Classic signal;
    any non-2xx/error is the negative signal), the `SettingsModel.addAccount` wiring, and the
    `AddAccountSheet` view. Live exercise is the local Classic fixture run (`make up
    BACKEND=classic`, `make install`, Add Account `http://localhost:8080` admin/admin).
