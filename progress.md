...
* [~] **Task 6.4:** the Gherkin layer — `.feature` files kept close to the client's
  ...
  * **Loop status:** the parser is done and fully tested headlessly — `GherkinParser`
    ...
    so sequencing / argument-passing / fail-fast are proven with a stub action. **Blocked:** only
    the *action bodies* each definition triggers (drive Task 6.3's `BackendAdmin` +
    `NSFileProviderManager` harness) remain Mac + Docker + signing gated; the step-text →
    definition mapping **and** the run orchestration are now both pure and proven.
